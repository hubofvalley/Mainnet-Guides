...
if [ "$VERSION_ID" = "22.04" ]; then
    ...
    mkdir -p story-geth
    cd story-geth
    wget -O v1.1.1.tar.gz https://github.com/piplabs/story-geth/archive/refs/tags/v1.1.1.tar.gz
    ...
    mkdir -p story-geth-v1.1.1
    wget -O story-geth-v1.1.1/geth-linux-amd64 https://github.com/piplabs/story-geth/releases/download/v1.1.1/geth-linux-amd64
    cp story-geth-v1.1.1/geth-linux-amd64 $HOME/go/bin/geth
    sudo chown -R $USER:$USER $HOME/go/bin/geth
    sudo chmod +x $HOME/go/bin/geth
else
...
wget -O $HOME/story-v1.3.1/story https://github.com/piplabs/story/releases/download/v1.3.1/story-linux-amd64
cp story-v1.3.1/story $HOME/go/bin/story
...
[Service]
User=$USER
ExecStart=$(which geth) --story --syncmode full --http --http.api eth,net,web3,engine --http.vhosts '*' --http.addr 127.0.0.1 --http.port ${STORY_PORT}545 --ws --ws.api eth,web3,net,txpool --ws.addr 127.0.0.1 --ws.port ${STORY_PORT}546 --authrpc.port ${STORY_PORT}551
StandardOutput=journal
StandardError=journal
Restart=on-failure
RestartSec=3
