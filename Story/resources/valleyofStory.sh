...
- current story node version: ${CYAN}v1.1.0 - v1.1.1 - v1.2.0${RESET}
...
function install_story_app() {
    ...
    wget -O story-v1.2.0/story https://github.com/piplabs/story/releases/download/v1.2.0/story-linux-amd64
    cp story-v1.2.0/story $HOME/go/bin/story
...
function show_guidelines() {
    ...
    echo -e "${GREEN}Install Story App only: Installs the Story app (v1.2.0) for executing transactions without running the node.${RESET}"
...
function menu() {
    ...
    echo -e "${GREEN}4. Install the Story App (v1.2.0) only to execute transactions without running a node${RESET}"
