...
- current story node version: ${CYAN}v1.1.1${RESET}
...
- peer: ${BLUE}fffb1a0dc2b6af331c65328c1ed9afad0bf107de@lightnode-peer-mainnet-story.grandvalleys.com:38656${RESET}
...
function query_balance() {
    ...
    case $choice in
        1)
            ...
            geth --exec "(parseFloat(eth.getBalance('$(story validator export | grep -oP '(?<=EVM Address: ).*')') / 1e18).toFixed(2)) + ' IP'" attach $HOME/.story/geth/story/geth.ipc
            ;;
        2)
            read -p "Enter the EVM address to query: " evm_address
            echo -e "${GREEN}Querying balance of $evm_address...${RESET}"
            geth --exec "(parseFloat(eth.getBalance('$evm_address') / 1e18).toFixed(2)) + ' IP'" attach ~/.story/geth/story/geth.ipc
            ;;
        3)
            menu
            ;;
...
function install_story_app() {
    ...
    wget -O story-v1.1.1/story https://github.com/piplabs/story/releases/download/v1.1.1/story-linux-amd64
...
function show_guidelines() {
    ...
    echo -e "${GREEN}Install Story App only: Installs the Story app (v1.1.1) for executing transactions without running the node.${RESET}"
...
function menu() {
    ...
    echo -e "${GREEN}4. Install the Story App (v1.1.1) only to execute transactions without running a node${RESET}"
