...
function create_validator() {
    ...
    read -p "Enter the commission rate: (e.g. 0.5) " COMMISSION_RATE

    read -p "Enter the max commission rate change: (e.g. 0.5) " MAX_COMMISSION_RATE_CHANGE
    ...
    namadac init-validator --email "\$EMAIL" --commission-rate "\$COMMISSION_RATE" --name "\$NAME" --max-commission-rate-change "\$MAX_COMMISSION_RATE_CHANGE" --account-keys \$WALLET_NAME --chain-id \$NAMADA_CHAIN_ID
    menu
}
...
function show_guidelines() {
    ...
    echo "   c. Show Wallet: Displays the wallet details."
    echo "   d. Query Balance: Queries the balance of a wallet."
    echo "   e. Create Validator: Creates a new validator."
    ...
    echo "   k. Claim Rewards: Claims rewards for staking."
    echo "   l. Vote Proposal: Votes on a proposal."
    echo "   m. Create Another Shielded Payment Address: Creates another shielded payment address."
    echo "   n. Transfer (Shielding): Transfers tokens to a shielded address."
    echo "   o. Transfer (Shielded to Shielded): Transfers tokens between shielded addresses."
    echo "   p. Transfer (Unshielding): Transfers tokens from a shielded address to a transparent address."
...
function menu() {
    ...
    echo -e "${GREEN}Let's Buidl Namada Together, Let's Shield Together. - Grand Valley${RESET}"
