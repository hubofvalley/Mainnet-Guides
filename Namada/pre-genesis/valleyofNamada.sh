...
function stake_tokens() {
    ...
    if [ "$RPC_CHOICE" == "2" ]; then
        story validator stake --validator-pubkey $VALIDATOR_PUBKEY --stake $AMOUNT $PRIVATE_KEY_FLAG --rpc https://lightnode-json-rpc-mainnet-namada.grandvalleys.com:443 --chain-id 1516
...
function unstake_tokens() {
    ...
    if [ "$RPC_CHOICE" == "2" ]; then
        story validator unstake --validator-pubkey $VALIDATOR_PUBKEY --unstake $AMOUNT $PRIVATE_KEY_FLAG --rpc https://lightnode-json-rpc-mainnet-namada.grandvalleys.com:443 --chain-id 1516
    elif [ "$RPC_CHOICE" == "1" ]; then
...
# Menu function
function menu() {
    echo -e "${CYAN}Namada Validator Node = Consensus Client Service + Execution Client Service (geth/story-geth)${RESET}"
    echo "Menu:"
